...
@section('title', $pageContent->meta_title ?? 'Contact Us')
...
@section('og_image', asset($pageContent->meta_image ?? 'og-home.jpg'))
    ...
    @section('content')
        <section class="container py-5">
            <div class="row">
                <h1 class="text-center">Contact Design Dhaga - Custom Fashion & Branding Inquiries</h1>
            </div>
            <div class="row py-3 justify-content-center">
                <div class="col-md-10 contact-details col-sm-12">
                    <h2><strong>Our Office Address</strong> : {{ $settings->office_address }}</h2>
                    <h3><strong>Our Physical Store</strong> : {{ $settings->store_address }}</h3>
                    <h4><strong>Phone No</strong> : <a href="tel:{{ $settings->contact_number }}">{{ $settings->contact_number }}</a> | {{ $settings->opening_time }} - {{ $settings->closing_time }} ({{ $settings->working_days }})</h4>
                    <h4><strong>Email Address</strong> : <a href="mailto:{{ $settings->support_email }}">{{ $settings->support_email }}</a></h4>
                    <h4><strong>Chat With Us</strong> : <a href="https://wa.me/{{ $settings->whatsapp_number }}?text=" target="_blank">
                        <svg viewBox="0 0 50 50" height="34" width="34" y="0px" x="0px" xmlns="http://www.w3.org/2000/svg">
        ...
        <section class="container py-5" id="form">
            <div class="row">
                <h4 class="text-center form-heading">From Thought to Form</h4>
            </div>
